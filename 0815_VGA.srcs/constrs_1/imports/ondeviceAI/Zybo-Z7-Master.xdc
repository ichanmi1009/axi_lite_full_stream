...
##LEDs
#set_property -dict { PACKAGE_PIN M14   IOSTANDARD LVCMOS33 } [get_ports { led[0] }]; # AXI video transfer observed
#set_property -dict { PACKAGE_PIN M15   IOSTANDARD LVCMOS33 } [get_ports { led[1] }]; # AXI start-of-frame observed
#set_property -dict { PACKAGE_PIN G14   IOSTANDARD LVCMOS33 } [get_ports { led[2] }]; # AXI end-of-line observed
#set_property -dict { PACKAGE_PIN D18   IOSTANDARD LVCMOS33 } [get_ports { led[3] }]; # Framebuffer write observed
...
#set_property -dict { PACKAGE_PIN V8 IOSTANDARD LVCMOS33 } [get_ports { ov7670_data[1] }];
#set_property -dict { PACKAGE_PIN W8 IOSTANDARD LVCMOS33 } [get_ports { ov7670_data[3] }];
#set_property -dict { PACKAGE_PIN U7 IOSTANDARD LVCMOS33 } [get_ports { ov7670_data[5] }];
#set_property -dict { PACKAGE_PIN V7 IOSTANDARD LVCMOS33 } [get_ports { ov7670_data[7] }];
#set_property -dict { PACKAGE_PIN Y7 IOSTANDARD LVCMOS33 } [get_ports { ov7670_data[0] }];
#set_property -dict { PACKAGE_PIN Y6 IOSTANDARD LVCMOS33 } [get_ports { ov7670_data[2] }];
#set_property -dict { PACKAGE_PIN V6 IOSTANDARD LVCMOS33 } [get_ports { ov7670_data[4] }];
#set_property -dict { PACKAGE_PIN W6 IOSTANDARD LVCMOS33 } [get_ports { ov7670_data[6] }];  
## Pmod Header JB - camera control
...
## BEGIN OV7670 PCLK TIMING CONSTRAINTS (remove this block to roll back)
# OV7670 data/HREF are launched from the falling edge of PCLK. At the current
# direct-clock setting PCLK follows the 25.20161 MHz VGA/XCLK source.
create_clock -name ov7670_pclk -period 39.680 -waveform {0.000 19.840} [get_ports {pclk}]
set_input_delay -clock ov7670_pclk -clock_fall -max 5.000 \
    [get_ports {d[*] href vsync}]
set_input_delay -clock ov7670_pclk -clock_fall -min 0.000 \
    [get_ports {d[*] href vsync}]
## END OV7670 PCLK TIMING CONSTRAINTS

## BEGIN OV7670/VGA CDC CONSTRAINT (remove this block to roll back)
# PCLK returns through the external sensor and has no guaranteed phase
# relationship to the FPGA VGA clock, even though their nominal rates match.
set_clock_groups -asynchronous \
    -group [get_clocks ov7670_pclk] \
    -group [get_clocks {sys_clk_pin clk_out1_clk_wiz_0}]
## END OV7670/VGA CDC CONSTRAINT

## BEGIN ILA ASYNCHRONOUS-PROBE CONSTRAINTS
# The ILA samples the OV7670 input pins with the 125 MHz board clock.  These
# probe-only paths are intentionally asynchronous and must not be timed as
# 125 MHz data paths.  Functional camera logic must still synchronize inputs
# before using them in the clk domain.
set_false_path -from [get_ports {d[*] pclk vsync href}] \
               -to [get_cells -hierarchical -filter {NAME =~ U_ILA_0/*}]
## END ILA ASYNCHRONOUS-PROBE CONSTRAINTS
...
#set_property -dict { PACKAGE_PIN F20   IOSTANDARD LVCMOS33 PULLUP true } [get_ports { cam_scl }]; #IO_L15N_T2_DQS_AD12N_35 Sch=cam_scl
#set_property -dict { PACKAGE_PIN F19   IOSTANDARD LVCMOS33 PULLUP true } [get_ports { cam_sda }]; #IO_L15P_T2_DQS_AD12P_35 Sch=cam_sda
